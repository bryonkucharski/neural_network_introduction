...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dA2, dW2, db2 = affine_backward(dZ2, cache2)
        ...
        grads['W2'] += 0.5*self.reg  * W2 *2
        grads['W1'] += 0.5*self.reg  * W1 * 2
        ...
        for l in range(self.num_layers):
            ...
            if l == self.num_layers -1: #output layer
                ...
                if self.normalization == "batchnorm":
                    ...
                    a, cache = affine_batchnorm_relu_forward(a,W,b,gamma,beta,self.bn_params[l-1])
                    if self.use_dropout:
                        a, dropout_cache[str(l + 1)] = dropout_forward(a,self.dropout_param)
                elif self.normalization=='layernorm':
                    gamma = self.params['gamma' + str(l + 1)]
                    beta = self.params['beta' + str(l + 1)]
                    a, cache = affine_layernorm_relu_forward(a,W,b,gamma,beta,self.bn_params[l-1])
                    if self.use_dropout:
                        a, dropout_cache[str(l + 1)] = dropout_forward(a,self.dropout_param)
                else:       
        ...
        for l in reversed(range(self.num_layers)):
            ...
            if self.normalization == "batchnorm":
                if l == self.num_layers -1: #output layer
                    ...
                    if self.use_dropout:
                        dA = dropout_backward(dA,dropout_cache[str(l + 1)])
            ...
            grads['W' + str(l + 1)] = dW
            grads['b' + str(l + 1)] = db
            grads['W' + str(l + 1)] += 0.5*self.reg  * W *2
